...
| repop_time            | If the density is lower than the upper bound the repop timer will be active and encounters will spawn. It will reset and stop once the density count is over the limit set. Essentially when the population count is incremented.                                                                                                                                                                     |
...
| relief_time           | Guaranteed time to next spawn. This assumes that the current population is below the `density` and `max_battle_size` values.                                                                                                                                                                                                                 |
